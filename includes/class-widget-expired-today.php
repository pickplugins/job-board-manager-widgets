...
class WidgetExpiredToday extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		echo '<ul class="job_bm_expired_today">';
		
		if ( $wp_query->have_posts() ) :

            do_action('job_bm_expired_today_jobs_before');
			...
			while ( $wp_query->have_posts() ) : $wp_query->the_post();
                ...
                <li>
                    ...
                    do_action('job_bm_expired_today_jobs_loop', $job_id);
            ...
            do_action('job_bm_expired_today_jobs_after');
		endif;
		wp_reset_query();
...
add_action('job_bm_expired_today_jobs_loop','job_bm_expired_today_jobs_loop', 10, 1);

if(!function_exists('job_bm_expired_today_jobs_loop')){
    function job_bm_expired_today_jobs_loop($job_id){
...
add_action('job_bm_expired_today_jobs_after','job_bm_expired_today_jobs_after', 10);


if(!function_exists('job_bm_expired_today_jobs_after')){
    function job_bm_expired_today_jobs_after($job_id){

        ?>

        <style type="text/css">
            .job_bm_expired_today{}
            .job_bm_expired_today li{
                margin: 0;
                padding: 0;
                list-style: none;
            }
            .job_bm_expired_today .single {
                clear: both;
                display: block;
                margin: 15px 0;
            ...
            .job_bm_expired_today .company_logo {
                width: 50px;
                height: 50px;
                overflow: hidden;
                float: left;
            ...
            .job_bm_expired_today .title {
                font-size: 15px;

            }

            .job_bm_expired_today a {
                text-decoration: none;

            }
            .job_bm_expired_today .company-name {
                display: inline-block;
                margin-right: 10px;
            }


            .job_bm_expired_today .job-meta {
                /*display: inline-block;*/
            }
            .job_bm_expired_today .job-meta span{
                display: inline-block;
                margin-right: 15px;
                font-size: 12px;
            }
